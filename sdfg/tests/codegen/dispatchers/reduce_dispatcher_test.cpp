...
TEST(SequentialReduceDispatcherTest, DispatchNode) {
    ...
    codegen::SequentialReduceDispatcher
    ...
    EXPECT_EQ(main_stream.str(), "// Reduce\nfor(i = 0;i < 10;i = 1 + i)\n{\n}\n");
}

TEST(ReduceDispatcherTest, RedirectDispatch) {
    builder::StructuredSDFGBuilder builder("sdfg_a", FunctionType_CPU);
    auto& sdfg = builder.subject();
    auto& root = sdfg.root();
    ...
    auto& loop = builder.add_reduce(
        ...
        symbolic::add(symbolic::symbol("i"), symbolic::integer(1)),
        {structured_control_flow::ReductionInfo{structured_control_flow::ReductionOperation::Add, "i"}},
        structured_control_flow::ScheduleType("mock_reduce", structured_control_flow::ScheduleTypeCategory::None)
    );
    ...
    codegen::ReduceDispatcherRegistry::instance().register_reduce_dispatcher(
        ...
        [&](codegen::LanguageExtension& language_extension,
            StructuredSDFG& sdfg,
            analysis::AnalysisManager& analysis_manager,
            structured_control_flow::Reduce& reduce,
