...
const std::vector<symbolic::Expression>& R2CFFT2DNode::shape() const { return this->shape_; }

types::PrimitiveType R2CFFT2DNode::real_primitive() const { return this->precision_; }

types::PrimitiveType R2CFFT2DNode::complex_primitive() const {
...
void R2CFFT2DNode::validate(const Function& function) const {
    data_flow::CodeNode::validate(function);

    if (this->shape_.size() != 3) {
        throw InvalidSDFGException("R2CFFT2DNode: shape must be [matrices, fftH, fftW]");
    }
    if (this->precision_ != types::PrimitiveType::Float && this->precision_ != types::PrimitiveType::Double) {
        throw InvalidSDFGException("R2CFFT2DNode: precision must be Float or Double");
    }
}

symbolic::SymbolSet R2CFFT2DNode::symbols() const {
...
void R2CFFT2DNode::replace(const symbolic::Expression old_expression, const symbolic::Expression new_expression) {
    for (auto& e : this->shape_) {
        e = symbolic::subs(e, old_expression, new_expression);
    }
}

void R2CFFT2DNode::replace(const symbolic::ExpressionMapping& replacements) {
    for (auto& e : this->shape_) {
        e = symbolic::subs(e, replacements);
    }
}

passes::LibNodeExpander::ExpandOutcome R2CFFT2DNode::
    expand(passes::LibNodeExpander::ExpandContext& context, structured_control_flow::Block& block) {
    // GPU-only node: realized by the CUDA/ROCm hand-tuned dispatchers, never expanded.
    return context.unable();
}

std::unique_ptr<data_flow::DataFlowNode> R2CFFT2DNode::
    clone(size_t element_id, const graph::Vertex vertex, data_flow::DataFlowGraph& parent) const {
    return std::unique_ptr<R2CFFT2DNode>(new R2CFFT2DNode(
        element_id, this->debug_info(), vertex, parent, this->implementation_type_, this->shape_, this->precision_
    ));
}

std::string R2CFFT2DNode::toStr() const {
    return LibraryNode::toStr() + "(r2c_fft2d, " + (this->precision_ == types::PrimitiveType::Double ? "d" : "s") + ")";
}

data_flow::PointerAccessType R2CFFT2DNode::pointer_access_type(int input_idx) const {
...
nlohmann::json R2CFFT2DNodeSerializer::serialize(const data_flow::LibraryNode& library_node) {
    const auto& node = static_cast<const R2CFFT2DNode&>(library_node);
    nlohmann::json j;
...
data_flow::LibraryNode& R2CFFT2DNodeSerializer::deserialize(
    ...
    if (j["code"].get<std::string>() != LibraryNodeType_R2CFFT2D.value()) {
    ...
    return builder.add_library_node<R2CFFT2DNode>(parent, debug_info, implementation_type, shape, precision);
}
