...
ElementWiseDataflowTensorNode::ElementOutput MinimumNode::expand_operation_dataflow(
    ...
    if (types::is_integer(input0.required_type)) {
        ...
        auto tasklet_code = TensorNode::get_integer_minmax_tasklet(input0.required_type, false);
        ...
        code_node = &builder.add_library_node<
            cmath::CMathNode>(block, this->debug_info(), cmath::CMathFunction::fmin, input0.required_type);
    }
...
std::unique_ptr<data_flow::DataFlowNode> MinimumNode::
    ...
    return std::unique_ptr<data_flow::DataFlowNode>(new MinimumNode(
