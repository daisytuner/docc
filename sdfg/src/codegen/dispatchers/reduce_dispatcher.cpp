...
SchedTypeReduceDispatcher::SchedTypeReduceDispatcher(
...
void SchedTypeReduceDispatcher::
    dispatch(PrettyPrinter& main_stream, PrettyPrinter& globals_stream, CodeSnippetFactory& library_snippet_factory) {
    auto dispatcher = ReduceDispatcherRegistry::instance().get_reduce_dispatcher(node_.schedule_type().value());
    if (dispatcher) {
        auto dispatcher_ptr =
        ...
        throw std::runtime_error("Unsupported reduce schedule type: " + std::string(node_.schedule_type().value()));
    }
};

InstrumentationInfo SchedTypeReduceDispatcher::instrumentation_info() const {
    auto dispatcher = ReduceDispatcherRegistry::instance().get_reduce_dispatcher(node_.schedule_type().value());
    if (dispatcher) {
        auto dispatcher_ptr =
            dispatcher(language_extension_, sdfg_, analysis_manager_, node_, instrumentation_plan_, arg_capture_plan_);
        auto reduce_dispatcher_ptr = static_cast<ReduceDispatcher*>(dispatcher_ptr.get());
        return reduce_dispatcher_ptr->instrumentation_info();
    } else {
        throw std::runtime_error("Unsupported reduce schedule type: " + std::string(node_.schedule_type().value()));
    }
};

SequentialReduceDispatcher::SequentialReduceDispatcher(
    LanguageExtension& language_extension,
    StructuredSDFG& sdfg,
    analysis::AnalysisManager& analysis_manager,
    structured_control_flow::Reduce& node,
...
void SequentialReduceDispatcher::dispatch_node(
...
InstrumentationInfo SequentialReduceDispatcher::instrumentation_info() const {
    ...
    return InstrumentationInfo(node_.element_id(), ElementType_Reduce, TargetType_SEQUENTIAL, loop_info, metrics);
};
