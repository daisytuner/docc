...
TEST(HIPFFTDispatcherTest, ForwardR2C_Float_WithTransfers) {
    ...
    std::string code =
        dispatch_fft(false, shape, symbolic::integer(4), types::PrimitiveType::Float, ImplementationType_ROCMWithTransfers);

    EXPECT_NE(code.find("hipfftPlanMany"), std::string::npos);
    EXPECT_NE(code.find("hipfftExecR2C"), std::string::npos);
    EXPECT_NE(code.find("HIPFFT_R2C"), std::string::npos);
    EXPECT_NE(code.find("hipMalloc"), std::string::npos);
    EXPECT_NE(code.find("hipMemcpy"), std::string::npos);
    EXPECT_NE(code.find("hipFree"), std::string::npos);
    // Hermitian odist for 8x8: 8 * (8/2+1) = 40.
    EXPECT_NE(code.find("40"), std::string::npos);
}

TEST(HIPFFTDispatcherTest, ForwardR2C_Float_WithoutTransfers) {
    std::vector<symbolic::Expression> shape = {symbolic::integer(8), symbolic::integer(8)};
    std::string code = dispatch_fft(
        false, shape, symbolic::integer(4), types::PrimitiveType::Float, ImplementationType_ROCMWithoutTransfers
    );

    EXPECT_NE(code.find("hipfftPlanMany"), std::string::npos);
    EXPECT_NE(code.find("hipfftExecR2C"), std::string::npos);
    EXPECT_EQ(code.find("hipMalloc"), std::string::npos);
}

TEST(HIPFFTDispatcherTest, ForwardR2C_Double_WithTransfers) {
    std::vector<symbolic::Expression> shape = {symbolic::integer(16), symbolic::integer(16)};
    std::string code = dispatch_fft(
        false, shape, symbolic::integer(2), types::PrimitiveType::Double, ImplementationType_ROCMWithTransfers
    );

    EXPECT_NE(code.find("hipfftExecD2Z"), std::string::npos);
    EXPECT_NE(code.find("HIPFFT_D2Z"), std::string::npos);
    EXPECT_NE(code.find("hipfftDoubleComplex"), std::string::npos);
}

TEST(HIPFFTDispatcherTest, InverseC2R_Float_WithTransfers) {
    std::vector<symbolic::Expression> shape = {symbolic::integer(8), symbolic::integer(8)};
    std::string code =
        dispatch_fft(true, shape, symbolic::integer(4), types::PrimitiveType::Float, ImplementationType_ROCMWithTransfers);

    EXPECT_NE(code.find("hipfftExecC2R"), std::string::npos);
    EXPECT_NE(code.find("HIPFFT_C2R"), std::string::npos);
}

TEST(HIPFFTDispatcherTest, InverseC2R_Double_WithoutTransfers) {
    std::vector<symbolic::Expression> shape = {symbolic::integer(8), symbolic::integer(8)};
    std::string code = dispatch_fft(
        true, shape, symbolic::integer(1), types::PrimitiveType::Double, ImplementationType_ROCMWithoutTransfers
    );

    EXPECT_NE(code.find("hipfftExecZ2D"), std::string::npos);
    EXPECT_EQ(code.find("hipMalloc"), std::string::npos);
}
