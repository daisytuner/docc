...
void expect_schedule_retained(
    ...
    EXPECT_EQ(schedule.value(), ScheduleType_CUDA::value());
    EXPECT_EQ(ScheduleType_CUDA::target_level(schedule), expected_level);
    EXPECT_EQ(ScheduleType_CUDA::parallel_size(schedule)->as_int(), expected_parallel_size);
