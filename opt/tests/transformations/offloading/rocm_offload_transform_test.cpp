// Unit tests for the (non-deprecated) ROCMOffloadTransform::can_be_applied.
//
...
// Counts ROCm data-offloading library nodes across the top-level blocks that
    ...
    for (size_t i = 0; i < root.size(); ++i) {
        ...
        if (auto* block = dynamic_cast<structured_control_flow::Block*>(&cf_node)) {
            for (auto& node : block->dataflow().nodes()) {
                if (dynamic_cast<ROCMDataOffloadingNode*>(&node) != nullptr) {
                    ++count;
                }
            }
        }
...
// Applies the transform and asserts the resulting map schedule is a ROCm
...
void expect_schedule_retained(
    ...
    ROCMOffloadTransform& transform,
    ...
    EXPECT_EQ(schedule.value(), ScheduleType_ROCM_Offload::value());
    EXPECT_EQ(ScheduleType_ROCM_Offload::target_level(schedule), expected_level);
    EXPECT_EQ(ScheduleType_ROCM_Offload::parallel_size(schedule)->as_int(), expected_parallel_size);

    // The offloaded argument must be surrounded by ROCm data-offloading nodes.
...
TEST(ROCMOffloadTransformTest, XGridWithinLimit) {
    builder::StructuredSDFGBuilder builder("rocm_xgrid_ok", FunctionType_CPU);
    auto& map = build_offloadable_map(builder, symbolic::integer(1024));

    analysis::AnalysisManager analysis_manager(builder.subject());
    ROCMOffloadTransform transform(map, symbolic::integer(kMaxGridX), gpu::TargetLevel::X_GRID);

    expect_schedule_retained(builder, analysis_manager, transform, map, gpu::TargetLevel::X_GRID, kMaxGridX);
}

TEST(ROCMOffloadTransformTest, YGridWithinLimit) {
    builder::StructuredSDFGBuilder builder("rocm_ygrid_ok", FunctionType_CPU);
    auto& map = build_offloadable_map(builder, symbolic::integer(1024));

    analysis::AnalysisManager analysis_manager(builder.subject());
    ROCMOffloadTransform transform(map, symbolic::integer(kMaxGridYZ), gpu::TargetLevel::Y_GRID);

    expect_schedule_retained(builder, analysis_manager, transform, map, gpu::TargetLevel::Y_GRID, kMaxGridYZ);
}

TEST(ROCMOffloadTransformTest, ZGridWithinLimit) {
    builder::StructuredSDFGBuilder builder("rocm_zgrid_ok", FunctionType_CPU);
    auto& map = build_offloadable_map(builder, symbolic::integer(1024));

    analysis::AnalysisManager analysis_manager(builder.subject());
    ROCMOffloadTransform transform(map, symbolic::integer(kMaxGridYZ), gpu::TargetLevel::Z_GRID);
...
TEST(ROCMOffloadTransformTest, XGridExceedsLimit) {
    builder::StructuredSDFGBuilder builder("rocm_xgrid_over", FunctionType_CPU);
    auto& map = build_offloadable_map(builder, symbolic::integer(1024));

    analysis::AnalysisManager analysis_manager(builder.subject());
    ROCMOffloadTransform transform(map, symbolic::integer(kMaxGridX + 1), gpu::TargetLevel::X_GRID);

    EXPECT_FALSE(transform.can_be_applied(builder, analysis_manager));
}

TEST(ROCMOffloadTransformTest, YGridExceedsLimit) {
    builder::StructuredSDFGBuilder builder("rocm_ygrid_over", FunctionType_CPU);
    auto& map = build_offloadable_map(builder, symbolic::integer(1024));

    analysis::AnalysisManager analysis_manager(builder.subject());
    ROCMOffloadTransform transform(map, symbolic::integer(kMaxGridYZ + 1), gpu::TargetLevel::Y_GRID);

    EXPECT_FALSE(transform.can_be_applied(builder, analysis_manager));
}

TEST(ROCMOffloadTransformTest, ZGridExceedsLimit) {
    builder::StructuredSDFGBuilder builder("rocm_zgrid_over", FunctionType_CPU);
    auto& map = build_offloadable_map(builder, symbolic::integer(1024));

    analysis::AnalysisManager analysis_manager(builder.subject());
    ROCMOffloadTransform transform(map, symbolic::integer(kMaxGridYZ + 1), gpu::TargetLevel::Z_GRID);
...
TEST(ROCMOffloadTransformTest, BlockTargetLevelRejected) {
    builder::StructuredSDFGBuilder builder("rocm_block_level", FunctionType_CPU);
    auto& map = build_offloadable_map(builder, symbolic::integer(1024));

    analysis::AnalysisManager analysis_manager(builder.subject());
    ROCMOffloadTransform transform(map, symbolic::integer(64), gpu::TargetLevel::X_BLOCK);

    EXPECT_FALSE(transform.can_be_applied(builder, analysis_manager));
}

TEST(ROCMOffloadTransformTest, WarpTargetLevelRejected) {
    builder::StructuredSDFGBuilder builder("rocm_warp_level", FunctionType_CPU);
    auto& map = build_offloadable_map(builder, symbolic::integer(1024));

    analysis::AnalysisManager analysis_manager(builder.subject());
    ROCMOffloadTransform transform(map, symbolic::integer(64), gpu::TargetLevel::WARP);
...
TEST(ROCMOffloadTransformTest, SmallParallelSizeAccepted) {
    builder::StructuredSDFGBuilder builder("rocm_small_parallel", FunctionType_CPU);
    auto& map = build_offloadable_map(builder, symbolic::integer(1024));

    analysis::AnalysisManager analysis_manager(builder.subject());
    ROCMOffloadTransform transform(map, symbolic::integer(64), gpu::TargetLevel::X_GRID);

    expect_schedule_retained(builder, analysis_manager, transform, map, gpu::TargetLevel::X_GRID, 64);
}

TEST(ROCMOffloadTransformTest, YGridParallelSizeBoundaryVsOverflow) {
    // Exactly at the limit is accepted, one over is rejected.
    {
        builder::StructuredSDFGBuilder builder("rocm_y_boundary", FunctionType_CPU);
        auto& map = build_offloadable_map(builder, symbolic::integer(1024));
        analysis::AnalysisManager analysis_manager(builder.subject());
        ROCMOffloadTransform transform(map, symbolic::integer(kMaxGridYZ), gpu::TargetLevel::Y_GRID);
        expect_schedule_retained(builder, analysis_manager, transform, map, gpu::TargetLevel::Y_GRID, kMaxGridYZ);
    ...
    {
        builder::StructuredSDFGBuilder builder("rocm_y_overflow", FunctionType_CPU);
        auto& map = build_offloadable_map(builder, symbolic::integer(1024));
        analysis::AnalysisManager analysis_manager(builder.subject());
        ROCMOffloadTransform transform(map, symbolic::integer(kMaxGridYZ + 1), gpu::TargetLevel::Y_GRID);
        EXPECT_FALSE(transform.can_be_applied(builder, analysis_manager));
...
TEST(ROCMOffloadTransformTest, StaticSizeAccepted) {
    builder::StructuredSDFGBuilder builder("rocm_static_size", FunctionType_CPU);
    auto& map = build_offloadable_map(builder, symbolic::integer(1024));
...
TEST(ROCMOffloadTransformTest, DynamicSizeAcceptedWhenAllowed) {
    builder::StructuredSDFGBuilder builder("rocm_dynamic_size", FunctionType_CPU);
    types::Scalar i64(types::PrimitiveType::Int64);
    builder.add_container("N", i64, /*is_argument=*/true);
    auto& map = build_offloadable_map(builder, symbolic::symbol("N"));

    analysis::AnalysisManager analysis_manager(builder.subject());
    ROCMOffloadTransform transform(map, symbolic::integer(1024), gpu::TargetLevel::X_GRID, /*allow_dynamic_sizes=*/true);

    expect_schedule_retained(builder, analysis_manager, transform, map, gpu::TargetLevel::X_GRID, 1024);
}

TEST(ROCMOffloadTransformTest, UnknownSizeRejected) {
    builder::StructuredSDFGBuilder builder("rocm_unknown_size", FunctionType_CPU);
