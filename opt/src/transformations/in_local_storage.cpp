...
bool InLocalStorage::can_be_applied(builder::StructuredSDFGBuilder& builder, analysis::AnalysisManager& analysis_manager) {
    ...
    // in global memory nor provide the cross-thread visibility a cooperative copy
    // needs, so we reject it rather than silently emit incorrect GPU code. Use
    // NV_Shared for GPU cooperative buffering, or CPU_Stack for per-thread /
    // host-local buffers.
    ...
    for (auto& memlet : dfg.out_edges(access_node_)) {
        ...
        // Require provably integer extents: a shared/local buffer needs a
        // compile-time size. Symbolic GPU map bounds are no longer substituted
        // with block sizes (that substitution silently assumed the map trip
        // count equalled the block size, which is unsafe — see the deadlock
        // safety check below).
