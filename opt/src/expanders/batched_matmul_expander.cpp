...
bool BatchedMatMulExpander::expand_batched_matmul(
    ...
    if (dataflow.in_degree(node) != 3 || dataflow.out_degree(node) != 0) {
        return false;
    }

    auto& layout_a = node.layout_a();
    auto& layout_b = node.layout_b();
    ...
    // Determine precision
    auto prim_type = node.uniform_quantization(dataflow);
    if (!prim_type) {
        return false;
    }
    ...
    // Get input edges
    auto iedges = dataflow.in_edges_by_connector(node);
    ...
    // Create access nodes
    auto& a_access = builder.add_access(gemm_block, input_node_a.data(), node.debug_info());
    auto& b_access = builder.add_access(gemm_block, input_node_b.data(), node.debug_info());
    auto& c_access = builder.add_access(gemm_block, output_ptr.data(), node.debug_info());
    auto& alpha_const = builder.add_constant(gemm_block, "1.0", scalar_type, node.debug_info());
    auto& beta_const = builder.add_constant(gemm_block, "0.0", scalar_type, node.debug_info());

    // Connect edges
    builder.add_computational_memlet(
        gemm_block, a_access, batched_gemm, "__A", {}, types::Pointer(scalar_type), node.debug_info()
    );
    builder.add_computational_memlet(
        gemm_block, b_access, batched_gemm, "__B", {}, types::Pointer(scalar_type), node.debug_info()
    );
    builder.add_computational_memlet(
        gemm_block, c_access, batched_gemm, "__C", {}, types::Pointer(scalar_type), node.debug_info()
    );
    builder
        .add_computational_memlet(gemm_block, alpha_const, batched_gemm, "__alpha", {}, scalar_type, node.debug_info());
    builder.add_computational_memlet(gemm_block, beta_const, batched_gemm, "__beta", {}, scalar_type, node.debug_info());

    // Clean up old block
    builder.clear_code_node_legacy(block, node);
