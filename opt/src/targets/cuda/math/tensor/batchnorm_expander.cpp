...
bool CudaBatchNormExpander::expand(builder::StructuredSDFGBuilder& builder, analysis::AnalysisManager& analysis_manager) {
    return expand_batch_norm(builder, analysis_manager, node_);
}

bool CudaBatchNormExpander::expand_batch_norm(
    ...
    auto batch_in = find_usable_input_access_node(dataflow, node, "Batch");
    auto& data_type = batch_in.memlet->base_type();
    types::Scalar scalar_type(data_type.primitive_type());
    types::Tensor tensor_1d(scalar_type, {node.num_features()}, {symbolic::one()});
    std::string temp_var_prefix = "_batchn_tmp";
    int tmp_idx = 0;
    auto var_in = find_usable_input_access_node(dataflow, node, "Var");
    auto e_in = find_usable_input_access_node(dataflow, node, "E");
    auto gamma_in = find_usable_input_access_node(dataflow, node, "Gamma");
    auto beta_in = find_usable_input_access_node(dataflow, node, "Beta");
    auto result_ptr_in = find_usable_input_access_node(dataflow, node, "B_out");
    auto eps_in = find_usable_input_access_node(dataflow, node, "epsilon");

    auto& new_sequence = builder.add_sequence_before(parent, block, transition.assignments(), node.debug_info());

    auto loop_dims = create_maps(builder, node.batch_layout().shape(), new_sequence);

    auto& c_dim = loop_dims.at(1);
    std::vector<symbolic::Expression> c_subset{c_dim.indvar};

    auto& innermost_dim = loop_dims.at(node.batch_layout().dims() - 1);
    ...
    // var[c] + eps
    auto& add_eps_op =
        builder.add_tasklet(innermost_block, data_flow::fp_add, "_out", {"var", "eps"}, node.debug_info());
    ...
    auto& sqrt_op = builder.add_library_node<math::cmath::CMathNode>(
        innermost_block, node.debug_info(), math::cmath::CMathFunction::sqrt, data_type.primitive_type()
    );
    ...
    // x - e[c]
    auto& sub_op = builder.add_tasklet(innermost_block, data_flow::fp_sub, "_out", {"x", "e"}, node.debug_info());
    ...
    auto& mul_interm_op =
        builder.add_tasklet(innermost_block, data_flow::fp_mul, "_out", {"num", "den"}, node.debug_info());
    ...
    // * gamma[c]
    auto& mul_gamma_op =
        builder.add_tasklet(innermost_block, data_flow::fp_mul, "_out", {"frac", "g"}, node.debug_info());
    ...
    // + beta[c]
    auto& add_beta_op =
        builder.add_tasklet(innermost_block, data_flow::fp_add, "_out", {"_in", "b"}, node.debug_info());
