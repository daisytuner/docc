...
BatchedGEMMNodeDispatcher_ROCMBLASWithTransfers::BatchedGEMMNodeDispatcher_ROCMBLASWithTransfers(
...
void BatchedGEMMNodeDispatcher_ROCMBLASWithTransfers::dispatch_code_with_edges(
    ...
    out.library_snippet_factory.add_global("#include <hipblas/hipblas.h>");
    ...
    switch (node.precision()) {
        ...
        case sdfg::math::blas::BLAS_Precision::d:
            ...
            break;
        default:
            throw std::runtime_error("Invalid precision for ROCMBLAS batched GEMM node");
    }
    ...
    out.stream << "err_hip = hipMalloc((void**) &dA, " << size_A << ");" << std::endl;
    rocm_error_checking(out.stream, this->language_extension_, "err_hip");
    out.stream << "err_hip = hipMalloc((void**) &dB, " << size_B << ");" << std::endl;
    rocm_error_checking(out.stream, this->language_extension_, "err_hip");
    out.stream << "err_hip = hipMalloc((void**) &dC, " << size_C << ");" << std::endl;
    rocm_error_checking(out.stream, this->language_extension_, "err_hip");

    out.stream << "err_hip = hipMemcpy(dA, " << a_expr << ", " << size_A << ", hipMemcpyHostToDevice);" << std::endl;
    rocm_error_checking(out.stream, this->language_extension_, "err_hip");
    out.stream << "err_hip = hipMemcpy(dB, " << b_expr << ", " << size_B << ", hipMemcpyHostToDevice);" << std::endl;
    rocm_error_checking(out.stream, this->language_extension_, "err_hip");
    out.stream << "err_hip = hipMemcpy(dC, " << c_expr << ", " << size_C << ", hipMemcpyHostToDevice);" << std::endl;
    rocm_error_checking(out.stream, this->language_extension_, "err_hip");
    ...
    out.stream << "err_hip = hipMemcpy(" << c_expr << ", dC, " << size_C << ", hipMemcpyDeviceToHost);" << std::endl;
    rocm_error_checking(out.stream, this->language_extension_, "err_hip");

    out.stream << "err_hip = hipFree(dA);" << std::endl;
    rocm_error_checking(out.stream, this->language_extension_, "err_hip");
    out.stream << "err_hip = hipFree(dB);" << std::endl;
    rocm_error_checking(out.stream, this->language_extension_, "err_hip");
    out.stream << "err_hip = hipFree(dC);" << std::endl;
    rocm_error_checking(out.stream, this->language_extension_, "err_hip");
...
void generate_kernel_batched_gemm(
    ...
    // hipBLAS is column-major native, so for row-major we swap A and B
    ...
    stream << "err = hipblas" << type << "gemmStridedBatched(handle, " << trans_first_str << ", " << trans_second_str
