...
passes::LibNodeExpander::ExpandOutcome GPUConvExpander::handle_expand_im2row(
    ...
    if (b.has_bias) {
        ...
        auto& B_access = standalone->add_indirect_read_access(transpose_block, math::tensor::ConvNode::B_INPUT_IDX);
        auto& Y_access = standalone->add_indirect_write_access(transpose_block, math::tensor::ConvNode::Y_INPUT_IDX);
        auto& tasklet =
            builder
