...
def _build_coop2d(N, R, K, block):
    """``out[i] += A[r,k]`` over a 2D tile (r,k), cooperative across the block dim i.

    Localizing A stages the whole [R][K] tile into shared memory (no per-thread
    slots); the transpose option stores it column-major. Result: out[i] = sum(A).
    """
    f = Scalar(PrimitiveType.Float)
    host = Pointer(f)
    dev = Pointer(f, StorageType.NV_Generic())
    ...
    b = StructuredSDFGBuilder("ls_coop2d_cuda")
    b.add_container("A", host, is_argument=True)
    b.add_container("out", host, is_argument=True)
    b.add_container("__daisy_dev_A", dev, is_argument=False)
    b.add_container("__daisy_dev_out", dev, is_argument=False)
    b.add_container("i", i32)
    b.add_container("r", i32)
    b.add_container("k", i32)
    ...
    outer = b.begin_for("r", "0", str(R), "1")
    b.begin_for("k", "0", str(K), "1")
    blk = b.add_block()
    o_in = b.add_access(blk, "__daisy_dev_out")
    a = b.add_access(blk, "__daisy_dev_A")
    o_out = b.add_access(blk, "__daisy_dev_out")
    t = b.add_tasklet(blk, TaskletCode.fp_add, ["_in1", "_in2"], ["_out"])
    b.add_memlet(blk, o_in, "", t, "_in1", "i", dev)
    b.add_memlet(blk, a, "", t, "_in2", f"r*{K} + k", dev)
    b.add_memlet(blk, t, "_out", o_out, "", "i", dev)
    b.end_for()
    b.end_for()
    b.end_map()
...
@pytest.mark.parametrize(
    "N,R,K,block",
    [(64, 4, 32, 32), (100, 8, 16, 32), (48, 4, 10, 32)],
    ids=["64x4x32", "100x8x16", "48x4x10"],
)
def test_local_storage_transposed(N, R, K, block, tmp_path):
    """Cooperative 2D tile staged column-major (``transpose_layout=True``): the
    shared buffer stores A[r,k] as buf[k][r]. The transpose is applied identically
    to the copy write and the compute read, so the reduction still equals sum(A)."""
    builder, outer, a = _build_coop2d(N, R, K, block)

    am = AnalysisManager(builder)
    xform = LocalStorage(outer, a, transpose_layout=True)
    assert xform.can_be_applied(builder, am), "transposed cooperative tile should apply"
    xform.apply(builder, am)
    SyncConditionPropagation().run(builder, am)

    sdfg = builder.move()
    sdfg.validate()
    output_dir = tmp_path / f"transposed_{N}x{R}x{K}"
    output_dir.mkdir(parents=True, exist_ok=True)
    lib_path = sdfg._compile(str(output_dir), "cuda")
...
@pytest.mark.parametrize(
    "N,M,K,bm,bn",
    [(8, 4, 16, 8, 4), (16, 8, 16, 8, 4), (10, 6, 8, 8, 4), (33, 5, 16, 8, 4)],
    ids=["8x4x16", "16x8x16", "10x6x8", "33x5x16"],
)
def test_local_storage_mixed_swizzle(N, M, K, bm, bn, tmp_path):
    """Same mixed kernel with ``swizzle_layout=True`` and a power-of-two tile (K):
    the shared A tile is XOR-swizzled (natural unpadded block) and staged by a
    per-thread-slot TileCopyNode carrying the swizzle on its dst. The XOR is a pure
    relabelling (applied identically to the copy write and the compute read), so the
    result must match the plain mixed path. Requires the slot to fit the block
    (bm <= K), the swizzle's validity precondition."""
    builder, inner, a = _build_mixed(N, M, K, bm, bn)

    am = AnalysisManager(builder)
    xform = LocalStorage(inner, a, swizzle_layout=True)
    assert xform.can_be_applied(builder, am), "swizzled mixed tile should apply"
    xform.apply(builder, am)
    SyncConditionPropagation().run(builder, am)

    sdfg = builder.move()
    sdfg.validate()
    output_dir = tmp_path / f"mixed_sw_{N}x{M}x{K}"
    output_dir.mkdir(parents=True, exist_ok=True)
    lib_path = sdfg._compile(str(output_dir), "cuda")
    ...
    # The swizzle placement must appear as an XOR in the addressing.
    assert "^" in generated, "expected an XOR-swizzled index"
...
def test_local_storage_lane_contiguous(N, K, block, tmp_path):
    """Cooperative tile with ``lane_contiguous=True``: A[0:K] is staged flat
    (Linearized, no padding) via a whole-block thread-linear sweep — the CDNA
    ``global_load_lds`` shape. Validates the flat staging path is numerically
    correct (on CUDA it degrades to a plain full-block cooperative copy)."""
    builder, inner, a = _build(N, K, block)

    am = AnalysisManager(builder)
    xform = LocalStorage(inner, a, lane_contiguous=True)
    assert xform.can_be_applied(builder, am), "lane-contiguous flat tile should apply"
    xform.apply(builder, am)
    SyncConditionPropagation().run(builder, am)

    sdfg = builder.move()
    sdfg.validate()
    output_dir = tmp_path / f"lanecontig_{N}x{K}x{block}"
    output_dir.mkdir(parents=True, exist_ok=True)
    lib_path = sdfg._compile(str(output_dir), "cuda")
