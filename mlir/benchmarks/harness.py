...
def _make_checked_docc_backend(target, remote_tuning, residency):
    from docc.torch.torch_program import TorchProgram
    ...
    def _checked_docc_backend(gm, example_inputs, *, options=None):
        ...
        resolved_inputs = [_resolve(inp) for inp in example_inputs]
        example_input = (
            resolved_inputs[0] if len(resolved_inputs) == 1 else tuple(resolved_inputs)
        )
